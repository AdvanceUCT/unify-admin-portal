...
```text
https://<admin-host>/activate?token=<opaque-token>
```
...
- The public `/activate` page hosts the clickable offer URL and falls back to `unifywallet://activate?token=<opaque-token>` when the Android App Link is not verified on the current device.
- Android App Links require `/.well-known/assetlinks.json` on the same admin host and a matching student-wallet package/certificate fingerprint.
  ...
  "activationDeliveries": [
    {
      ...
      "credentialId": "credential-demo-002",
      "studentId": "student-demo-002",
      "activationUrl": "https://admin.example.test/activate?token=opaque-token",
      "status": "Delivered",
