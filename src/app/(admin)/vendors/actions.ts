...
export async function revokeBranchPaymentAcceptanceAction(formData: FormData) {
  const session = await requireRole(["SUPER_ADMIN", "ADMIN"]);
  assertCan("vendor:write", session);

  const branchId = String(formData.get("branchId") ?? "");
  const { revokeBranchPaymentAcceptance } = await import("@/lib/payments/branchOnboarding");
