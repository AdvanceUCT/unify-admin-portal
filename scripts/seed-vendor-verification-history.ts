 ...
 * afterward. Pair with scripts/sql/reset-billing-demo-data.sql to run a
 * demo repeatedly from a clean slate.
 */
...
async function main() {
  ...
  const { resolveVerificationBillingSnapshot } = await import("../src/lib/vendors/verificationBilling");
  ...
  try {
    ...
    for (const vendor of vendors) {
      for (let i = 0; i < countPerVendor; i += 1) {
        const branch = vendor.branches.length > 0 ? vendor.branches[Math.floor(Math.random() * vendor.branches.length)] : null;
      ...
      console.log(`Seeded ${countPerVendor} verification(s) for ${vendor.companyName}.`);
    }
